...
export default function ImageCompressor() {
  ...
  const processFiles = async (selectedFiles) => {
    ...
    // Declare a variable to hold the original sizes
    let originalSizes = [];
    ...
    const newFiles = await Promise.all(
      validFiles.map(async (file) => {
        ...
        try {
          ...
          // Save original size in the variable
          originalSizes.push(file.size); // Store original size
          return {
            file: compressedFile,
          ...
          originalSizes.push(file.size); // Store original size in case of error
          return { file, format: file.type.split("/")[1] };
        }
      })
    );
    ...
    setFiles((prevFiles) => {
      ...
      setFileSizes((prevSizes) => {
        const newSizes = newFiles.reduce((acc, _, index) => {
          acc[prevFiles.length + index] = {
            original: originalSizes[index], // Use the variable to get the original size
            compressed: null, // Initial compressed size is null
          };
          return acc;
        }, {});
  ...
  const handleCompressAndDownload = async (index) => {
    ...
    const quality = 1 - level / 100; // Convert slider value (0-100) to quality
    ...
    try {
      const originalSizeMB = currentFile.originalSize / (1024 * 1024); // Convert to MB

      const options = {
        maxSizeMB: originalSizeMB > 1 ? 1 : originalSizeMB, // Only compress if larger than 1MB
        maxWidthOrHeight: 1920,
        useWebWorker: true,
        initialQuality: quality, // Use quality based on slider
      };
      ...
      setDownloadUrls((prevUrls) => ({
        ...
        [index]: {
          ...
          size: processedFile.size, // Save compressed size
        },
      }));

      // Update compressed file size in fileSizes state
      setFileSizes((prevSizes) => ({
        ...prevSizes,
        [index]: {
          original:  fileSizes[index]?.original,
          compressed: processedFile.size, // Save compressed size separately
        },
      }));
    } catch (error) {
              ...
              <div className="flex md:flex-row flex-col-reverse items-center md:gap-3 gap-2 justify-center md:p-2 p-1">
                ...
                <div className="flex flex-col md:w-auto items-center">
                  ...
                  <Typography className="text-xs md:text-sm mt-1 font-kalame-regular ">
                    حجم اصلی: {(fileSizes[index]?.original / 1024).toFixed(2)}{" "}
                    KB
                  </Typography>

                 {fileSizes[index]?.compressed &&
                   <Typography className="text-xs md:text-sm mt-1 font-kalame-regular ">
                   حجم بعد از کاهش:{" "}
                   {(fileSizes[index]?.compressed / 1024).toFixed(2)} KB
                 </Typography>
                 }
                </div>
